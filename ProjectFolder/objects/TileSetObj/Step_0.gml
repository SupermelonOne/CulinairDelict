if (global.generated && !doneTileset){
	...
	{
		//y = 0
		if (place_meeting(x-16, y-16, TileSetObj)){
			array_set(surroundingTiles[0], 0, 1); 
		}
		if (place_meeting(x, y-16, TileSetObj)){
		...
		//y = 1
		if (place_meeting(x-16, y, TileSetObj)){
			array_set(surroundingTiles[1], 0, 1); 
		}
		if (place_meeting(x, y, TileSetObj)){
...
//MOST SPECIFIC
	...
	[ 1,1,1 ] 
		...
		if (irandom_range(0, 100) <= 2){
			var randomTile = choose(1, 2, 3, 4, 5, 6)
			switch(randomTile){
				case 1:
					image_index = 47;
					break;
