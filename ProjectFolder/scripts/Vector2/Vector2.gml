function vector(_x, _y) constructor {
	...
	static rotate_radian = function(_angle) {
		gml_pragma("forceinline");
		x = x * cos(_angle) - y * sin(_angle);
		y = x * sin(_angle) + y * cos(_angle);
	}
	
	static rotate = function (_degree) {
		gml_pragma("forceinline");
		var rad = (pi / 180) * _degree;
		rotate_radian(rad);
	}
	...
	static get_angle_rad = function() {
		return arctan2(x, y);
	}
	static get_angle_deg = function() {
		return arctan2(x, y) / pi * 360;
	}
